...
async def serve_dashboard(request: Request, db: Session = Depends(get_db)):
    try:
        ...
        # 👉 Use effective_id to pull synchronized data if account is linked
...
async def get_live_chat(request: Request, last_id: int = 0, db: Session = Depends(get_db)):
    """Polls local database for chat messages to save YouTube API Quota."""
    try:
        streamer_id = request.session.get("streamer_id")
        if not streamer_id:
            return {"messages": []}
        ...
        # PULL FROM ALL CHANNELS OWNED BY THIS USER
...
async def link_secondary_account(
    ...
    """Links the current logged-in streamer account to a primary account using its sync code."""
    try:
        ...
        if not primary_streamer:
            return RedirectResponse(url="/?error=invalid_sync_code", status_code=303)

        if primary_streamer.id == current_streamer.id:
            return RedirectResponse(url="/?error=cannot_link_self", status_code=303)
        ...
        logger.exception(f"[ACCOUNT LINK ERROR] Failed linking account: {e}")
...
async def unlink_account(request: Request, db: Session = Depends(get_db)):
    """Unlinks the current account, restoring its independent settings."""
    try:
        current_streamer_id = request.session.get("streamer_id")
        if not current_streamer_id: return RedirectResponse(url="/", status_code=303)
...
async def serve_admin_panel(request: Request, admin_user: str = Depends(verify_admin), db: Session = Depends(get_db)):
    try:
        ...
        return templates.TemplateResponse(
            ...
            context={"request": request, "admin_name": admin_user.capitalize(), "total_users": total_users, "total_streamers": total_streamers, "all_streamers": all_streamers, "active_video_ids": active_video_ids}
        )
    except Exception as e:
        return RedirectResponse(url="/", status_code=303)
...
async def admin_disconnect_stream(request: Request, video_id: str = Form(...), admin_user: str = Depends(verify_admin), db: Session = Depends(get_db)):
    try:
        if video_id in DETECTED_VIDEOS:
            del DETECTED_VIDEOS[video_id]
        DISCONNECT_QUEUE.add(video_id)
            ...
            if video_id_element is not None:
                ...
                if channel_id_element is not None:
                    ...
                    if streamer and streamer.server_sync_code:
                        DETECTED_VIDEOS[video_id] = streamer.id
                        await overlay_manager.send_alert(streamer.server_sync_code, {
                            "type": "alert", "event_type": "newSponsorEvent", "author": "🤖 SYSTEM CONNECTED",
                            "message": "mod hajir hai janab uk malik ki kami nhi hone dega 😁😸 (Mods type !checkup)", "amount": "✅ ONLINE"
                        })
...
async def render_overlay(request: Request, sync_code: str):
    try:
        ...
        return templates.TemplateResponse(
            request=request, name="overlay.html", 
...
async def test_alert(request: Request, db: Session = Depends(get_db)):
    try:
        ...
        if streamer_id:
            ...
            if streamer and streamer.server_sync_code:
                test_payload = {
                    "type": "alert", "event_type": "superChatEvent", "author": "System Tester",
                    "message": "This is a test Super Chat! Your OBS widget is working perfectly!", "amount": "$50.00"
                }
...
async def custom_alert(request: Request, alert_title: str = Form(...), alert_message: str = Form(...), db: Session = Depends(get_db)):
    try:
        ...
        if streamer_id:
            ...
            if streamer and streamer.server_sync_code:
                custom_payload = {
                    "type": "alert", "event_type": "newSponsorEvent", "author": alert_title,
                    "message": alert_message, "amount": "📢 ANNOUNCEMENT"
                }
...
async def update_goal(request: Request, goal_id: int = Form(...), amount: int = Form(...), db: Session = Depends(get_db)):
    try:
        ...
        if goal:
            ...
            if streamer and streamer.server_sync_code:
                await overlay_manager.send_alert(streamer.server_sync_code, {
                    "type": "goal_update", "goal_id": goal.id, "current": goal.current_amount, "target": goal.target_amount
                })
...
async def add_custom_command(
    request: Request, command_trigger: str = Form(...), response_text: str = Form(...), db: Session = Depends(get_db)
):
    try:
        streamer_id = request.session.get("streamer_id")
        if not streamer_id: return RedirectResponse(url="/", status_code=303)
        ...
        streamer = db.query(Streamer).filter(Streamer.id == streamer_id).first()
        effective_id = streamer.effective_id if streamer else streamer_id

        clean_trigger = command_trigger.strip().lower()
        if not clean_trigger.startswith("!"):
            clean_trigger = f"!{clean_trigger}"

        new_cmd = CustomCommand(streamer_id=effective_id, command_trigger=clean_trigger, response_text=response_text)
        db.add(new_cmd)
        db.commit()
...
async def delete_custom_command(request: Request, command_id: int = Form(...), db: Session = Depends(get_db)):
    try:
        ...
        if streamer_id:
            streamer = db.query(Streamer).filter(Streamer.id == streamer_id).first()
            effective_id = streamer.effective_id if streamer else streamer_id
            cmd = db.query(CustomCommand).filter(CustomCommand.id == command_id, CustomCommand.streamer_id == effective_id).first()
            if cmd:
                db.delete(cmd)
                db.commit()
        return RedirectResponse(url="/", status_code=303)
...
async def add_vip_guest(
    request: Request, target_username: str = Form(...), custom_reply: str = Form(...), db: Session = Depends(get_db)
):
    try:
        streamer_id = request.session.get("streamer_id")
        if not streamer_id: return RedirectResponse(url="/", status_code=303)
        ...
        new_vip = VIPGuest(streamer_id=effective_id, target_username=clean_username, custom_reply=custom_reply)
        db.add(new_vip)
        db.commit()
...
async def delete_vip_guest(request: Request, vip_id: int = Form(...), db: Session = Depends(get_db)):
    try:
        ...
        if streamer_id:
            streamer = db.query(Streamer).filter(Streamer.id == streamer_id).first()
            effective_id = streamer.effective_id if streamer else streamer_id
            vip = db.query(VIPGuest).filter(VIPGuest.id == vip_id, VIPGuest.streamer_id == effective_id).first()
            if vip:
                db.delete(vip)
                db.commit()
        return RedirectResponse(url="/", status_code=303)
...
async def startup_event():
    try:
        ...
        logger.info("[STARTUP] All services are ACTIVE!")
    except Exception as e:
